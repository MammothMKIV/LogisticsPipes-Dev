...
import logistics_bc.transport.pipes.*;
...
public abstract class Pipe implements IDropControlInventory {
//IPipe, 
...
//	public boolean broadcastSignal[] = new boolean[]{false, false, false, false};
	...
	public Pipe(PipeTransport transport, PipeLogic logic, int itemID) {
		...
		if (!networkWrappers.containsKey(this.getClass())) {
			networkWrappers
					.put(this.getClass(), new TilePacketWrapper(new Class[]{lp_TileGenericPipe.class, this.transport.getClass(), this.logic.getClass()}));
		}
	}

	public void setTile(TileEntity tile) {

		this.container = (lp_TileGenericPipe) tile;

		transport.setTile((lp_TileGenericPipe) tile);
		logic.setTile((lp_TileGenericPipe) tile);
	}
...
//		updateSignalState();
	...
	public boolean canPipeConnect(TileEntity tile, ForgeDirection side) {
		...
		if (tile instanceof lp_TileGenericPipe) {
			otherPipe = ((lp_TileGenericPipe) tile).pipe;
...
/*
	private void readNearbyPipesSignal(WireColor color) {
		...
		for (ForgeDirection o : ForgeDirection.VALID_DIRECTIONS) {
			...
			if (tile instanceof lp_TileGenericPipe) {
				lp_TileGenericPipe tilePipe = (lp_TileGenericPipe) tile;
...
/*
	public int isPoweringTo(int l) {
		...
		if (tile instanceof lp_TileGenericPipe && Utils.checkPipesConnections(this.container, tile))
			return 0;
	...
	protected void notifyBlocksOfNeighborChange(ForgeDirection side) {
		container.worldObj.notifyBlocksOfNeighborChange(container.xCoord + side.offsetX, container.yCoord + side.offsetY, container.zCoord + side.offsetZ, lp_BuildCraftTransport.genericPipeBlock.blockID);
	}

	protected void updateNeighbors(boolean needSelf) {
		if (needSelf) {
			container.worldObj.notifyBlocksOfNeighborChange(container.xCoord, container.yCoord, container.zCoord, lp_BuildCraftTransport.genericPipeBlock.blockID);
		}
	...
	public boolean isNearbyTriggerActive(ITrigger trigger, ITriggerParameter parameter) {
		if (trigger instanceof ITriggerPipe)
			...
			for (ForgeDirection o : ForgeDirection.VALID_DIRECTIONS) {
				...
				if (tile != null && !(tile instanceof lp_TileGenericPipe)) {
...
/* old IPipe
	@Override
	public lp_TileGenericPipe getContainer() {
		return container;
	}*/
/*
	@Override
