...
import logistics_bc.core.network.ISyncedTile;
import logistics_bc.core.network.PacketTileState;
import logistics_bc.core.utils.Utils;
import logistics_bc.transport.network.PipeRenderStatePacket;
...
public class lp_TileGenericPipe extends TileEntity implements IPowerReceptor, IFluidHandler, IPipeEntry, ITileBufferHolder,
		IPipeConnection, IDropControlInventory, IPipeRenderState, ISyncedTile, ISolidSideTile {
//IOverrideDefaultTriggers, IPipeTile, 
	...
	public void readFromNBT(NBTTagCompound nbttagcompound) {
		...
		if (pipe != null) {
			...
			lp_BuildCraftCore.bcLog.log(Level.WARNING, "Pipe failed to load from NBT at {0},{1},{2}", new Object[]{xCoord, yCoord, zCoord});
			deletePipe = true;
		}
...
*/
	...
	public void initialize(Pipe pipe) {
		...
		if (pipe == null) {
			lp_BuildCraftCore.bcLog.log(Level.WARNING, "Pipe failed to initialize at {0},{1},{2}, deleting", new Object[]{xCoord, yCoord, zCoord});
			worldObj.setBlockToAir(xCoord, yCoord, zCoord);
			return;
		}
		...
		for (ForgeDirection o : ForgeDirection.VALID_DIRECTIONS) {
			...
			if (tile instanceof ITileBufferHolder) {
				((ITileBufferHolder) tile).blockCreated(o, lp_BuildCraftTransport.genericPipeBlock.blockID, this);
			}
...
/*
	...
	protected boolean arePipesConnected(TileEntity with, ForgeDirection side) {
		...
		if (with instanceof lp_TileGenericPipe) {
			//if (((lp_TileGenericPipe) with).hasPlug(side.getOpposite()))
			//	return false;
			Pipe pipe2 = ((lp_TileGenericPipe) with).pipe;
