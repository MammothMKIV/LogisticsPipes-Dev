...
public class BlockGenericPipe extends BlockContainer {
	...
	public float getBlockHardness(World par1World, int par2, int par3, int par4) {
		return lp_BuildCraftTransport.pipeDurability;
	}
	...
	public void addCollisionBoxesToList(World world, int i, int j, int k, AxisAlignedBB axisalignedbb, List arraylist, Entity par7Entity) {
		...
		lp_TileGenericPipe tileG = (lp_TileGenericPipe) tile1;
...
*/
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer entityplayer, int side, float xOffset, float yOffset, float zOffset) {
		...
		world.notifyBlocksOfNeighborChange(x, y, z, lp_BuildCraftTransport.genericPipeBlock.blockID);
...
*/
	...
	public static Pipe createPipe(int key) {
		...
		try {
			...
			if (pipe != null)
				...
				lp_BuildCraftCore.bcLog.warning("Detected pipe with unknown key (" + key + "). Did you remove a buildcraft addon?");
			}

		} catch (Throwable t) {
			lp_BuildCraftCore.bcLog.warning("Failed to create pipe with (" + key + "). No valid constructor found. Possibly a item ID conflit.");
		}
	...
	public static boolean placePipe(Pipe pipe, World world, int i, int j, int k, int blockId, int meta) {
		...
		if (placed) {
			...
			lp_TileGenericPipe tile = (lp_TileGenericPipe) world.getBlockTileEntity(i, j, k);
	...
	public static Pipe getPipe(IBlockAccess blockAccess, int i, int j, int k) {
		...
		if (!(tile instanceof lp_TileGenericPipe) || tile.isInvalid())
			return null;

		return ((lp_TileGenericPipe) tile).pipe;
	}
...
//		lp_BuildCraftCore.instance.gateIconProvider.registerIcons(iconRegister);
//		lp_BuildCraftCore.instance.wireIconProvider.registerIcons(iconRegister);
...
/*
	...
	public Icon getIcon(int par1, int par2) {
		return lp_BuildCraftCore.instance.pipeIconProvider.getIcon(PipeIconProvider.TYPE.Stripes.ordinal());
	}*/
		...
		for (int i = 0; i < its; ++i) {
			for (int j = 0; j < its; ++j) {
				for (int k = 0; k < its; ++k) {
					...
					EntityDiggingFX fx = new EntityDiggingFX(worldObj, px, py, pz, px - x - 0.5D, py - y - 0.5D, pz - z - 0.5D, lp_BuildCraftTransport.genericPipeBlock, random, meta);
					fx.func_110125_a(icon);
