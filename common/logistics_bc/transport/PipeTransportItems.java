...
import logistics_bc.core.inventory.Transactor;
import logistics_bc.core.network.PacketIds;
import logistics_bc.core.proxy.CoreProxy;
import logistics_bc.core.utils.BlockUtil;
import logistics_bc.core.utils.Utils;
import logistics_bc.transport.network.PacketPipeTransportContent;
//import logistics_bc.transport.network.PacketPipeTransportNBT;
//import logistics_bc.transport.network.PacketSimpleId;
...
public class PipeTransportItems extends PipeTransport {
	...
	public void entityEntering(IPipedItem item, ForgeDirection orientation) {
		...
		if (data == null) {
			...
			if (item.getContainer() != null && item.getContainer() != this.container) {
				((PipeTransportItems) ((lp_TileGenericPipe) item.getContainer()).pipe.transport).scheduleRemoval(item);
			}
		...
		if (!container.worldObj.isRemote) {
			...
			if (travelingEntities.size() > lp_BuildCraftTransport.groupItemsTrigger) {
				groupEntities();
			}

			if (travelingEntities.size() > MAX_PIPE_STACKS) {
				lp_BuildCraftCore.bcLog.log(Level.WARNING, String.format("Pipe exploded at %d,%d,%d because it had too many stacks: %d", container.xCoord, container.yCoord, container.zCoord, travelingEntities.size()));
				destroyPipe();
				return;
			}
			...
			if (numItems > MAX_PIPE_ITEMS) {
				lp_BuildCraftCore.bcLog.log(Level.WARNING, String.format("Pipe exploded at %d,%d,%d because it had too many items: %d", container.xCoord, container.yCoord, container.zCoord, numItems));
				destroyPipe();
				return;
			}
		}
	...
	public boolean canReceivePipeObjects(ForgeDirection o, IPipedItem item) {
		...
		if (entity instanceof IPipeEntry)
			return true;
		else if (entity instanceof lp_TileGenericPipe) {
	...
	private void handleTileReached(EntityData data, TileEntity tile) {
		...
		} else if (tile instanceof lp_TileGenericPipe && ((lp_TileGenericPipe) tile).pipe.transport instanceof PipeTransportItems) {
	...
	public boolean canPipeConnect(TileEntity tile, ForgeDirection side) {
		if (tile instanceof lp_TileGenericPipe) {
			Pipe pipe2 = ((lp_TileGenericPipe) tile).pipe;
			if (BlockGenericPipe.isValid(pipe2) && !(pipe2.transport instanceof PipeTransportItems))
				return false;
		}

		return tile instanceof lp_TileGenericPipe || tile instanceof IPipeEntry || tile instanceof ISpecialInventory || (tile instanceof IInventory && ((IInventory) tile).getSizeInventory() > 0)
