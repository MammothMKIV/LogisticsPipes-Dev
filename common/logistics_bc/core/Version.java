...
public class Version implements Runnable {
	...
	public static boolean needsUpdateNoticeAndMarkAsSeen() {
		...
		Property property = lp_BuildCraftCore.mainConfiguration.get("vars", "version.seen", VERSION);
	...
	public static void versionCheck() {
		try {
				...
				if (line.startsWith(mcVersion)) {
					if (line.contains(DefaultProps.MOD)) {
						...
						if (line.endsWith(VERSION)) {
						    lp_BuildCraftCore.bcLog.finer("Using the latest version [" + getVersion() + "] for Minecraft " + mcVersion);
			...
			lp_BuildCraftCore.bcLog.warning("Using outdated version [" + VERSION + " (build:" + BUILD_NUMBER + ")] for Minecraft " + mcVersion
					+ ". Consider updating.");
			currentVersion = EnumUpdateState.OUTDATED;

		} catch (Exception e) {
			lp_BuildCraftCore.bcLog.warning("Unable to read from remote version authority.");
			lp_BuildCraftCore.bcLog.warning(e.toString());
	...
	public static String[] grabChangelog(String version) {
		...
		try {
			...
			ex.printStackTrace();
			lp_BuildCraftCore.bcLog.warning("Unable to read changelog from remote site.");
		}
    ...
    public void run() {
        ...
        lp_BuildCraftCore.bcLog.info("Beginning version check");
        ...
        try {
            while ((count < 3) && ((currentVersion == null) || (currentVersion == EnumUpdateState.CONNECTION_ERROR))) {
                ...
                if (currentVersion == EnumUpdateState.CONNECTION_ERROR) {
                    lp_BuildCraftCore.bcLog.info("Version check attempt " + count + " failed, trying again in 10 seconds");
                    Thread.sleep(10000);
                }
            }
        }
        ...
        if (currentVersion == EnumUpdateState.CONNECTION_ERROR) {
            lp_BuildCraftCore.bcLog.info("Version check failed");
        }
