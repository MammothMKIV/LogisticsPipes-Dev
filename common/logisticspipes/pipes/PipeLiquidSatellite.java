...
//TODO 	MainProxy.sendPacketToServer(new PacketPipeInteger(NetworkConstants.HUD_STOP_WATCHING, getX(), getY(), getZ(), 1).getPacket());
	...
	public void playerStartWatching(EntityPlayer player, int mode) {
		if(mode == 1) {
			...
			final ModernPacket packet = PacketHandler.getPacket(SatPipeSetID.class).setSatID(((BaseLogicLiquidSatellite)this.logic).satelliteId).setPosX(container.xCoord).setPosY(container.yCoord).setPosZ(container.zCoord);
